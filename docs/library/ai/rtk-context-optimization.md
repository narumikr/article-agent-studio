...
### 主要LLM APIのキャッシュ課金の比較（2026年5月時点）

> **注意**: 入力/出力単価は頻繁に変更されるため、本ドキュメントでは固定値を掲載せず、各公式ページの確認を前提とします。
...
| モデル | キャッシュ読み取り | キャッシュ書き込み |
|--------|-------------------|--------------------|
| Claude Opus | 入力の10% | 入力の125% |
| Claude Sonnet | 入力の10% | 入力の125% |
| Claude Haiku | 入力の10% | 入力の125% |
| GPT-4.1 | 自動50%割引 | - |
| Gemini 2.5 Flash | 通常の10% | - |

LLM APIは一般に入力・出力の両方に課金されるため、コンテキストが大きいほど総コストは増加します。
そのうえで各社のキャッシュ課金ルールには差があり、Anthropicでは書き込み時に追加コストがかかる一方、同一プロンプトの繰り返し利用では大幅な節約が見込めます。
